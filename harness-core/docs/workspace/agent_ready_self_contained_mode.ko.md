...
Agent-ready export mode에는 `.git` metadata가 없으므로 `node tools/checks/workspace/check_current_state_alignment.mjs`를 기본 명령으로 사용하지 않는다. 해당 checker는 root workspace mode 전용이다.
