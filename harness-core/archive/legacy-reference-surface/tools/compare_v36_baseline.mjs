...
const workspaceRoot = path.basename(v2Root) === "harness-core" ? path.dirname(v2Root) : repoRoot;
...
const disallowedSnapshotPaths = [...new Set([...inventoryPaths, ...snapshotByPath.keys()])]
  .filter((p) => EXCLUDED_SNAPSHOT_BASENAMES.includes(path.basename(p)))
  .sort();
...
const unexplainedItems = notInExistingRecord
  ...
  .concat(disallowedSnapshotPaths.map((p) => ({ path: p, explanation: "path is excluded by v36 baseline snapshot policy but is present in inventory or checksums" })))
...
const result = {
  ...
  alpha_snapshot: {
    ...
    disallowed_snapshot_path_count: disallowedSnapshotPaths.length
  },
  snapshot_exclusion_policy: {
    id: SNAPSHOT_EXCLUSION_POLICY_ID,
    excluded_basenames: EXCLUDED_SNAPSHOT_BASENAMES,
    disallowed_snapshot_paths: disallowedSnapshotPaths,
    source_files_removed: false,
    reason: "macOS Finder metadata is ignored for v36 baseline snapshot inventory/checksums."
  },
