...
- `baseline-snapshotted`: reference baseline inventory and checksums exist.
...
- `evidence-lineage-indexed`: evidence lineage was summarized from the reference baseline through beta evidence bundle draft.
...
## Local-Model-Verified Scoped Claim

`local-model-verified` means the post-stable Ollama qwen local lane passed the final local model verification gate after the owner approved the exact final decision phrase. The scope is limited to qwen3:14b and qwen3.6:27b evidence for no-tool, multi-model comparison, bounded local redteam, dependency-backed Ollama adapter conformance, redaction/storage audit, and owner-approved reference baseline integrity with the `.DS_Store` snapshot exclusion policy enforced.

It allows:
- `local-model-verified` claim only for the post-stable Ollama qwen local lane
...
Additional rules:
- `local-model-verified` is not provider diversity.
- `local-model-verified` is not provider verification.
- `local-model-verified` is not an adapter-checked claim.
- `local-model-verified` is not production readiness.
- `local-model-verified` is not stable release readiness.
- `local-model-verified` does not allow bare `release-gated`.
- The final gate does not run a new local generation, local redteam rerun, OpenAI provider call, telemetry sink write, local endpoint probe, production deployment, release gate rerun, or additional reference baseline refresh.

## Local-Model-Verified Final Handoff Claim

`post-stable-local-model-verified-final-handoff-recorded` means the scoped `local-model-verified` state for the Ollama qwen3 local lane was recorded into final handoff/archive artifacts with final claim state, evidence pointer index, archive manifest, archive checksums, strict provider paths, reference baseline status, and next options.

It allows:
- final handoff statement for the Ollama qwen3 local lane
- archive manifest statement
- final claim state statement
- strict provider path registry statement
...
Additional rules:
- final handoff/archive is not new local model execution
- final handoff/archive is not provider diversity
- final handoff/archive is not provider verification
- final handoff/archive is not adapter checking
- final handoff/archive is not production-ready or stable
- final handoff/archive does not refresh `evidence/reference-baseline`
- final handoff/archive does not modify the legacy reference source

## Combined OpenAI Local Archive Export Claim

`combined-openai-local-archive-export-recorded` means the OpenAI-only post-RC scoped stable archive and the Ollama qwen3 local-model-verified archive were indexed into one combined archive/export packet with final claim state, evidence pointer index, archive manifest, archive checksums, strict paths, reference baseline status, and next options.

It allows:
- combined archive/export statement
- combined final claim state statement
- combined evidence pointer index statement
- combined strict paths registry statement
...
Additional rules:
- combined archive/export is not new local model execution
- combined archive/export is not OpenAI provider execution
- combined archive/export is not telemetry sink write
- combined archive/export is not a local endpoint probe
- combined archive/export is not provider diversity
- combined archive/export is not provider verification
- combined archive/export is not adapter checking
- combined archive/export is not a general production-ready or stable release state
- combined archive/export does not refresh `evidence/reference-baseline`
- combined archive/export does not modify the legacy reference source or dist

## Post-Combined Strict Paths Autopilot Claims

`post-combined-provider-diverse-path-designed` means the provider diversity criteria and lane mapping were drafted after the combined archive. It is a design-only claim.

`post-combined-provider-diverse-inventory-recorded` means the candidate lane inventory was recorded for owner review. It is not a strong claim allowance.

`post-combined-provider-verified-gate-designed` means provider-level verification criteria were drafted without executing a provider verification gate.

`post-combined-adapter-checked-gate-designed` means adapter coverage criteria and gaps were drafted without executing a full adapter coverage gate.

`post-combined-strict-paths-owner-decision-packet-recorded` means the strict paths owner decision packet was recorded with readiness and blocker state.

`provider-diverse` means the post-combined provider diversity final gate passed after owner approval for the OpenAI API lane and Ollama qwen3 local lane. The scope is limited to evidence lineage showing two distinct provider lanes: OpenAI API post-RC scoped stable evidence and Ollama qwen3 local-model-verified evidence.

`post-combined-provider-diverse-final-gate-passed` means the provider diversity final gate evaluated the existing OpenAI API lane and Ollama qwen3 local lane evidence without new model execution.

`post-combined-provider-diverse-claim-enabled` means only the scoped `provider-diverse` claim is enabled by that final gate.

`post-combined-provider-diverse-owner-final-decision-recorded` means the owner approval phrase for the provider diversity final gate is recorded.

`post-combined-provider-diverse-final-decision-recorded` means the final decision record enables `provider-diverse` and keeps stronger adjacent claims blocked.

`post-combined-provider-diverse-evidence-accepted` means the required OpenAI API lane, Ollama qwen3 lane, lane independence, combined archive, inventory preflight, source archive, protected boundary, and no-new-execution evidence checks passed.

`post-combined-provider-diverse-archive-refreshed` means the combined archive/export claim state was refreshed after the provider diversity final gate so that the scoped `provider-diverse` allowance is reflected.

`post-combined-provider-diverse-final-claim-state-recorded` means the provider-diverse archive refresh recorded the final claim state with `provider-diverse` allowed and adjacent stronger claims blocked.

`post-combined-provider-diverse-evidence-indexed` means the provider-diverse final gate, path design, evidence inventory, strict paths owner packet, combined archive, local-model-verified handoff, and OpenAI-only stable handoff pointers were indexed.

`post-combined-provider-diverse-next-gates-recorded` means the next candidate gates for provider verification, adapter checking, and final export execution were recorded without opening those claims.

`post-combined-provider-diverse-final-export-draft-refreshed` means the final export draft state was refreshed to reflect provider diversity without writing an actual export package or modifying dist.

`post-combined-provider-verified-gate-preflight-completed` means provider verification criteria and lane evidence gaps were assessed without enabling `provider-verified`.

`post-combined-provider-verified-evidence-inventoried` means OpenAI API and Ollama qwen3 local provider verification evidence was indexed for owner review without enabling `provider-verified`.

`post-combined-provider-verified-owner-decision-packet-recorded` means a provider verification owner packet was recorded with strong claims still blocked.

`post-combined-adapter-checked-gate-preflight-completed` means adapter checking criteria, coverage, and gaps were assessed without enabling `adapter-checked`.

`post-combined-adapter-checked-coverage-matrix-recorded` means OpenAI, Ollama, vLLM placeholder, and common adapter coverage/gaps were recorded.

`post-combined-adapter-checked-owner-decision-packet-recorded` means an adapter checking owner packet was recorded with strong claims still blocked.

`post-combined-provider-adapter-final-owner-packet-recorded` means provider verification and adapter checking readiness were combined into a final owner packet while keeping both claims blocked.

`final-export-execution-preflight-recorded` means final export execution readiness and approval requirements were recorded without writing the actual export or modifying dist.

`final-export-execution-recorded` means the explicitly approved final export execution created a bounded zip package under `harness-core/exports/` and recorded its manifest, checksums, evidence index, package record, and claim boundary without modifying `dist`.

`final-export-execution-completed` means the final export execution gate passed for the bounded `exports/` package.

`final-export-package-created` means the final export zip exists and excludes `node_modules/`, `dist/`, `.git/`, `.DS_Store`, logs, raw payloads, and secret values.

`final-export-manifest-recorded`, `final-export-checksums-recorded`, and `final-export-claim-boundary-recorded` mean the corresponding export records exist under `evidence/final-export-execution/`.

`final-export-package-draft-recorded` means an export package draft manifest and checksum record were written without modifying dist or writing an actual export package.

These allow:
- design-only path statement
- inventory statement
- owner decision packet statement
- scoped `provider-diverse` statement after final gate
- provider-diverse archive refresh statement
- provider-diverse final claim state statement
- provider-diverse evidence index statement
- next gates registry statement
- provider verification preflight and owner-packet statements
- adapter checking preflight and owner-packet statements
- provider/adapter final owner-packet statement
- final export execution preflight statement
- final export execution package statement
- final export draft statement
...
Additional rules:
- strict path autopilot does not run OpenAI model calls
- strict path autopilot does not run local model generation
- strict path autopilot does not write telemetry
- strict path autopilot does not refresh `evidence/reference-baseline`
- strict path autopilot does not modify the legacy reference source, dist, node_modules, or package dependencies
- final export execution may create `harness-core/exports/*.zip` after the exact approval phrase is recorded
- final export execution must not modify `dist`
