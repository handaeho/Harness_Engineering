...
const REQUIRED_FILES = [
  ...
  "tools/builders/release/build_post_rc_new_conversation_handoff.mjs",
  "tools/checks/release/check_post_rc_new_conversation_handoff.mjs",
...
const scopeYaml = readTextIfExists("release/scopes/post-rc/post_rc_new_conversation_handoff_scope.yaml");
const checks = [];
...
const promptStart = "이 대화는 기존 harness-core 작업을 이어받는 새 대화입니다.\n아래 내용을 현재 확정 상태로 사용해 주세요.";
...
addCheck(checks, "NEW_CONVERSATION_PROMPT.ko.md is paste-ready and has required ending",
  promptDoc.startsWith(promptStart)
    ...
    && promptDoc.includes("node tools/checks/release/check_post_rc_new_conversation_handoff.mjs"),
  {});
...
addCheck(checks, "scope yaml records approved handoff actions and forbidden execution",
  scopeYaml.includes(`stage: ${STAGE}`)
    ...
    && scopeYaml.includes("additional_reference_baseline_refresh: true"),
  {});
