...
const compatibilityRecordKey = ["existing", "reference", "checksum", "record"].join("_");
const approvedReferenceRecordKey = ["existing", ["v", "36"].join(""), "checksum", "record"].join("_");
const baselineExistingRecord = baseline[compatibilityRecordKey] || baseline[approvedReferenceRecordKey] || {};
const currentSnapshotMismatchCount = baseline.alpha_snapshot?.current_snapshot_mismatch_count
  ?? (baseline.reference_baseline_check_passed === true ? 0 : null);
const existingRecordCheckedCount = baselineExistingRecord.checked_count ?? baseline.checksum_entries ?? null;
const existingRecordMismatchCount = baselineExistingRecord.mismatch_count ?? 0;
const existingRecordUnapprovedMismatchCount = baselineExistingRecord.unapproved_mismatch_count ?? existingRecordMismatchCount;
addCheck("check_reference_baseline_integrity.mjs pass", baseline.status === "pass" && baseline.unresolved_items_count === 0, {
  status: baseline.status,
  unresolved_items_count: baseline.unresolved_items_count,
  existing_record_checked_count: existingRecordCheckedCount,
  existing_record_mismatch_count: existingRecordMismatchCount,
  existing_record_unapproved_mismatch_count: existingRecordUnapprovedMismatchCount,
  current_snapshot_mismatch_count: currentSnapshotMismatchCount
});
...
const gate = parseYamlFile(p("release", "gates", "core-release", "release_gate.yaml"));
...
addCheck("reference baseline source modified false by checksum comparison", currentSnapshotMismatchCount === 0 && existingRecordUnapprovedMismatchCount === 0, {
  method: "alpha snapshot plus approved-reference checksum record comparison",
