...
function runReferenceBaselineIntegrity() {
  if (!hasPath("tools/check_reference_baseline_integrity.mjs")) {
    return {
      status: "fail",
      checker_exit_code: null,
      checker_stdout_json: null,
      checker_stderr: "tools/check_reference_baseline_integrity.mjs missing"
    };
  }
  const result = spawnSync(process.execPath, ["tools/check_reference_baseline_integrity.mjs"], {
    cwd: root,
    encoding: "utf8",
...
const importLeaks = [
  ...
  ...findExternalImports("tools/check_reference_baseline_integrity.mjs").map((item) => ({ file: "tools/check_reference_baseline_integrity.mjs", ...item }))
];
...
const referenceBaselineSnapshotCheck = {
  ...
  checker: "tools/check_reference_baseline_integrity.mjs",
...
if (!hasPath(cleanExportCheckPath)) {
  writeJson(cleanExportCheckPath, {
    ...
    stage: STAGE,
    generated_at: report.generated_at,
    note: "Run node tools/check_clean_export_self_contained.mjs from the source workspace to inspect the clean export archive."
  });
}
