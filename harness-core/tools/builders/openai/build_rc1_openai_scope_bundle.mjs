...
function evidenceIndex(root) {
  return [
    evidenceEntry(root, "reference baseline", "pass", "baseline_snapshot", true, [
      { path: "evidence/reference-baseline/file_inventory.json" },
      { path: "evidence/reference-baseline/checksums.json" }
    ]),
...
function evidenceLineage() {
  return [
    lineageEntry({
      ...
      evidencePaths: ["evidence/reference-baseline/file_inventory.json", "evidence/reference-baseline/checksums.json"]
    }),
    lineageEntry({
...
function manifest(root, report, index) {
  ...
  return {
    ...
    excludes: [
      "node_modules",
      "dist",
      "legacy-reference-source mutation",
...
\`\`\`powershell
node harness-core/tools/checks/openai/check_rc1_openai_scope_bundle.mjs
\`\`\`
`;
}
...
function writeArtifacts(root, values) {
  ...
  writeSnapshot(root, "release/gates/core-release/release_gate.yaml", "evidence/rc1-openai-scope-bundle/rc1_release_gate_snapshot.yaml");
