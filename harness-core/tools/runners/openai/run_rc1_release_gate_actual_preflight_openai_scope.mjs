...
export function prerequisiteState(root) {
  ...
  const notStableExists = exists(root, "release/notices/rc1/rc1_not_stable_notice.yaml")
...
function commandPlanYaml() {
  ...
  commands:
    - node harness-core/tools/runners/openai/run_rc1_release_gate_actual_openai_scope.mjs
    - node harness-core/tools/checks/openai/check_rc1_release_gate_actual_openai_scope.mjs
...
function rollbackReadiness(root) {
  const rollbackPlanExists = exists(root, "release/plans/core-release/rollback_plan.md") || exists(root, "release/plans/core-release/rollback_plan_draft.yaml");
  const draft = readYamlIfExists(root, "release/plans/core-release/rollback_plan_draft.yaml", {});
  const targets = Array.isArray(draft?.rollback_targets) ? draft.rollback_targets : [];
  const markdown = readTextIfExists(root, "release/plans/core-release/rollback_plan.md", "");
...
function ownerActionReadiness(root) {
  const matrixExists = exists(root, "release/decisions/general/owner_action_matrix.yaml");
  const matrix = readYamlIfExists(root, "release/decisions/general/owner_action_matrix.yaml", {});
...
\`\`\`powershell
node tools/checks/openai/check_rc1_release_gate_actual_preflight_openai_scope.mjs C:\\WORK\\0.개인\\HARNESS\\harness-core
\`\`\`
`;
}
...
- Gate script: \`tools/checks/openai/check_rc1_release_gate_actual_preflight_openai_scope.mjs\`
