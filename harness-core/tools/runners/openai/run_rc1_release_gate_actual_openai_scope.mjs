...
function prerequisiteState(root) {
  ...
  const approvalGate = readYamlIfExists(root, "release/gates/rc1/rc1_release_gate_actual_approval_gate.yaml", {});
...
\`\`\`powershell
node tools/checks/openai/check_rc1_release_gate_actual_openai_scope.mjs C:\\WORK\\0.개인\\HARNESS\\harness-core
\`\`\`
`;
}
...
- Gate script: \`tools/checks/openai/check_rc1_release_gate_actual_openai_scope.mjs\`
...
export function buildRc1ActualGateArtifacts(root = resolveRoot()) {
  ...
  writeText(p(root, "release", "gates", "core-release", "release_gate.yaml"), releaseGateYaml(report));
