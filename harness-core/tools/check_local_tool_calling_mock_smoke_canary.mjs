...
const REQUIRED = [
  "local_tool_calling_mock_smoke_report.json",
  "local_tool_calling_mock_response_mapping.json",
  "local_tool_calling_mock_redaction_report.json",
  "local_tool_calling_mock_claim_boundary.json",
  "local_tool_calling_mock_blocker_update.json",
  "local_tool_calling_mock_gate_report.json",
...
const report = fs.existsSync(e("local_tool_calling_mock_smoke_report.json"))
  ? readJson(e("local_tool_calling_mock_smoke_report.json"))
  : null;
const boundary = fs.existsSync(e("local_tool_calling_mock_claim_boundary.json"))
  ? readJson(e("local_tool_calling_mock_claim_boundary.json"))
  : null;
const redaction = fs.existsSync(e("local_tool_calling_mock_redaction_report.json"))
  ? readJson(e("local_tool_calling_mock_redaction_report.json"))
  : null;
...
addCheck(checks, "new local generation call count bounded", report?.new_local_model_execution === true
  && report?.new_local_generation_calls === 2, { calls: report?.new_local_generation_calls });
...
addCheck(checks, "cases passed", report?.cases_total === 2 && report?.cases_passed === 2 && report?.cases_failed === 0, {});
addCheck(checks, "tool calls present", report?.case_results?.every((item) => item.tool_calls_present === true
  && item.tool_name_matches === true), {});
addCheck(checks, "no external tool execution", report?.external_tool_executed === false
  && report?.case_results?.every((item) => item.external_tool_executed === false), {});
...
addCheck(checks, "strong claims blocked", boundary?.local_model_verified_allowed === false
  ...
  && boundary?.tool_call_verified_allowed === false, boundary || {});
...
const scan = scanClaims(root, {
  excludedPaths: [
    ...
    "node_modules",
    ".git",
    "evals/reports/local_tool_calling_mock_smoke_check_report.json",
    "evals/reports/local_tool_calling_mock_smoke_check_report.md"
  ]
});
...
const gate = {
  ...
  can_proceed_to_replay_regression_smoke: failed.length === 0,
...
const md = `# Local Tool-calling Mock Smoke Canary Check

Status: ${gate.status}

- Stage: ${STAGE}
- New local generation calls: ${gate.new_local_generation_calls}
- Can proceed to replay/regression smoke: ${gate.can_proceed_to_replay_regression_smoke}
...
writeJson(p("evals", "reports", "local_tool_calling_mock_smoke_check_report.json"), gate);
writeText(p("evals", "reports", "local_tool_calling_mock_smoke_check_report.md"), md);
