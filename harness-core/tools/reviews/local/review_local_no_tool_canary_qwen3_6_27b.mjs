...
const readinessPath = "evidence/post-rc-local-endpoint-readiness-preflight-qwen3-6-27b/local_endpoint_readiness_gate_report.json";
const readinessProbePath = "evidence/post-rc-local-endpoint-readiness-preflight-qwen3-6-27b/endpoint_probe_summary.json";
const canaryPath = "evidence/post-rc-local-no-tool-canary-qwen3-6-27b/local_no_tool_canary_gate_report.json";
const mappingPath = "evidence/post-rc-local-no-tool-canary-qwen3-6-27b/local_response_mapping_report.json";
const redactionPath = "evidence/post-rc-local-no-tool-canary-qwen3-6-27b/local_no_tool_redaction_report.json";
const tracePath = "evidence/post-rc-local-no-tool-canary-qwen3-6-27b/local_trace_samples.jsonl";
const qwen14ReviewPath = "evidence/post-stable-local-no-tool-canary-qwen3-14b-result-review/local_no_tool_canary_qwen3_14b_result_review.json";
const runnerPath = "tools/runners/local/run_local_no_tool_canary.mjs";

const requiredInputs = [readinessPath, readinessProbePath, canaryPath, mappingPath, redactionPath, tracePath, runnerPath];
...
const qwen14Review = existsRel(qwen14ReviewPath) ? readJsonRel(qwen14ReviewPath) : null;
const runnerSource = existsRel(runnerPath) ? fs.readFileSync(p(...runnerPath.split("/")), "utf8") : "";
...
const reasoningEffortNoneApplied = mapping?.request_mapping?.reasoning_effort_none_for_ollama === true
  && mappingCases.every((item) => item.request?.reasoning_effort === "none");
const reasoningEffortNestedNoneApplied = runnerSource.includes("requestBody.reasoning = { effort: \"none\" };")
  && mappingCases.every((item) => item.request?.reasoning_present === true);
const rawRequestStored = [
  canary?.raw_request_stored,
  redaction?.raw_request_stored,
  readiness?.raw_request_stored,
  readinessProbe?.raw_request_stored
].some((value) => value === true);
const rawResponseStored = [
  canary?.raw_response_stored,
  mapping?.response_mapping?.raw_response_stored,
  redaction?.raw_response_stored,
  readiness?.raw_response_stored,
  readinessProbe?.raw_response_stored
].some((value) => value === true);
const noRawStorage = rawRequestStored === false && rawResponseStored === false;
...
const reviewPass = missingInputs.length === 0
  ...
  && thinkFalseApplied
  && reasoningEffortNoneApplied
  && reasoningEffortNestedNoneApplied
  && noRawStorage
...
const resultReview = {
  ...
  reasoning_effort_none_applied: reasoningEffortNoneApplied,
  reasoning_effort_nested_none_applied: reasoningEffortNestedNoneApplied,
  final_content_non_empty: finalContentNonEmpty,
  raw_request_stored: rawRequestStored,
...
const canaryIndex = {
  ...
  evidence_files: [
    ...
    },
    {
      ...
      reasoning_effort_none_for_ollama: mapping?.request_mapping?.reasoning_effort_none_for_ollama || false,
...
const reasoningRecord = {
  ...
  model: MODEL,
  provider: PROVIDER,
  issue_observed: "think:false alone produced empty final content in first no-tool canary",
  fix_applied: [
    "think_false",
    "reasoning_effort_none",
    "reasoning_effort_nested_none"
  ],
  think_false_applied: thinkFalseApplied,
  reasoning_effort_none_applied: reasoningEffortNoneApplied,
  reasoning_effort_nested_none_applied: reasoningEffortNestedNoneApplied,
  result_after_fix: reviewPass
    ? "final content non-empty and canary passed within token budget"
    : "result review did not confirm final content and canary pass",
  current_mapping_evidence: mappingPath,
  runner_control_evidence: runnerPath,
  claim_impact: {
...
const storageReview = {
  ...
  raw_request_stored: rawRequestStored,
  raw_response_stored: rawResponseStored,
  secret_logged: redaction?.secrets_logged === true,
  auth_header_logged: redaction?.raw_authorization_header_recorded === true,
  api_key_logged: redaction?.api_key_recorded === true,
  redaction_passed: redaction?.status === "pass"
};
...
const claimBoundary = {
  status: reviewPass ? "pass" : "fail",
  stage: STAGE,
  local_no_tool_canary_qwen3_6_27b_allowed: reviewPass,
  ...
  reason: "qwen3.6:27b local no-tool path passed, but broader local model verification and provider-diverse claims require additional gates."
};

const blockerUpdate = {
  status: "updated",
  stage: STAGE,
  previous_status: "qwen3_14b_no_tool_reviewed_qwen3_6_27b_added",
  new_status: reviewPass
    ? "qwen3_14b_and_qwen3_6_27b_no_tool_paths_passed_multimodel_comparison_pending"
    : "qwen3_6_27b_no_tool_result_review_failed",
  unblocks: reviewPass
    ? [
      "post-stable-local-no-tool-canary-qwen3-6-27b-passed"
    ]
  ...
  next_required_actions: [
    "compare qwen3:14b and qwen3.6:27b local no-tool results",
    "define local-model verification gate",
    "define provider-diverse path gate"
  ]
};

const comparisonPreconditionsReady = qwen14Review?.status === "pass" && reviewPass;
const multimodelPreconditions = {
  status: comparisonPreconditionsReady ? "ready" : "blocked_by_missing_result_review",
  stage: STAGE,
  next_candidate_stage: "v2.0.0-post-stable-local-no-tool-multimodel-comparison-qwen3-14b-vs-qwen3-6-27b",
  models_ready: [
    "qwen3:14b",
    "qwen3.6:27b"
  ],
  ...
  qwen3_14b_result_review_passed: qwen14Review?.status === "pass",
  qwen3_6_27b_result_review_passed: reviewPass,
...
const unresolvedItems = reviewPass ? [] : [
  ...
  ...(!reviewPass && missingInputs.length === 0 ? [{
    id: "LNR-Q36-CHECKS",
    severity: "high",
    description: "One or more qwen3.6:27b local no-tool result review checks failed.",
...
const md = `# qwen3.6:27b Local No-tool Result Review
...
- reasoning_effort:none applied: ${resultReview.reasoning_effort_none_applied}
- reasoning.effort:none applied: ${resultReview.reasoning_effort_nested_none_applied}
...
const claimBoundaryMd = `# qwen3.6:27b Local No-tool Claim Boundary

Status: ${claimBoundary.status}

- local_no_tool_canary_qwen3_6_27b_allowed: ${claimBoundary.local_no_tool_canary_qwen3_6_27b_allowed}
- local_model_verified_allowed: false
- provider_diverse_allowed: false
- provider_verified_allowed: false
- adapter_checked_allowed: false
...
writeJson(p("evidence", EVIDENCE_DIR, "local_no_tool_canary_qwen3_6_27b_result_review.json"), resultReview);
...
writeJson(p("evidence", EVIDENCE_DIR, "qwen3_6_27b_reasoning_control_record.json"), reasoningRecord);
writeJson(p("evidence", EVIDENCE_DIR, "local_no_tool_storage_redaction_review.json"), storageReview);
writeJson(p("evidence", EVIDENCE_DIR, "local_no_tool_canary_qwen3_6_27b_claim_boundary.json"), claimBoundary);
writeJson(p("evidence", EVIDENCE_DIR, "local_no_tool_canary_qwen3_6_27b_blocker_update.json"), blockerUpdate);
writeJson(p("evidence", EVIDENCE_DIR, "multimodel_comparison_preconditions.json"), multimodelPreconditions);
writeJson(p("evidence", EVIDENCE_DIR, "local_no_tool_canary_qwen3_6_27b_gate_report.json"), gateReport);
writeJson(p("evidence", EVIDENCE_DIR, "unresolved_items.json"), unresolvedItems);

writeJson(p("evals", "reports", "local_no_tool_canary_qwen3_6_27b_result_review_report.json"), resultReview);
writeText(p("evals", "reports", "local_no_tool_canary_qwen3_6_27b_result_review_report.md"), md);
writeJson(p("evals", "reports", "local_no_tool_canary_qwen3_6_27b_claim_boundary_report.json"), claimBoundary);
writeText(p("evals", "reports", "local_no_tool_canary_qwen3_6_27b_claim_boundary_report.md"), claimBoundaryMd);
writeJson(p("evals", "reports", "local_no_tool_canary_qwen3_6_27b_gate_report.json"), gateReport);
writeText(p("evals", "reports", "local_no_tool_canary_qwen3_6_27b_gate_report.md"), md);
