...
const REQUIRED_FILES = [
  ...
  `${EVIDENCE_DIR}/final_v36_baseline_refresh_status.json`,
...
const finalV36BaselineRefreshStatus = readJsonIfExists(`${EVIDENCE_DIR}/final_v36_baseline_refresh_status.json`);
...
const baselineRefreshFiles = finalV36BaselineRefreshStatus?.owner_approved_refresh_files_still_modified_in_worktree || [];
addCheck(checks, "v36 baseline refresh status records prior owner-approved files only",
  finalV36BaselineRefreshStatus?.status === "recorded"
    && finalV36BaselineRefreshStatus?.compare_v36_baseline_status === "pass"
    && finalV36BaselineRefreshStatus?.v36_modified === false
    && finalV36BaselineRefreshStatus?.dist_modified === false
    && finalV36BaselineRefreshStatus?.additional_refresh_in_this_stage === false
    && includesAll(baselineRefreshFiles, PRIOR_BASELINE_REFRESH_FILES),
  finalV36BaselineRefreshStatus || {});
...
addCheck(checks, "forbidden execution flags remain false",
  falseFlags(finalHandoff, [
    ...
    "dist_modified",
    "additional_v36_baseline_refresh",
    "evidence_v36_baseline_modified_in_this_stage"
  ]),
  finalHandoff || {});
