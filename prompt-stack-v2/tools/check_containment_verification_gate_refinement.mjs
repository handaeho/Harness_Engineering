...
import { buildArtifacts, resolveRoot, STAGE } from "./refine_containment_verification_gate.mjs";
...
const mappingTool = runJsonTool("tools/map_containment_evidence_to_boundaries.mjs");
const proofAuditTool = runJsonTool("tools/audit_containment_proof_levels.mjs");
const validateAlpha = runJsonTool("tools/validate_alpha.mjs");
const compareBaseline = runJsonTool("tools/compare_v36_baseline.mjs");
const mockGate = runJsonTool("tools/check_containment_boundary_mock_dry_run.mjs");
...
for (const relPath of [
  "release/containment_verification_gate_refined.yaml",
  "release/containment_release_blocker_refresh.yaml",
  "release/containment_proof_requirements.yaml",
...
addCheck(checks, "v36 modified false by checksum comparison", compareBaseline.parsed?.alpha_snapshot?.current_snapshot_mismatch_count === 0
  && compareBaseline.parsed?.existing_v36_checksum_record?.mismatch_count === 0, {
