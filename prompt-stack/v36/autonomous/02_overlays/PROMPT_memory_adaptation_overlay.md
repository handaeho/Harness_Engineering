...
- one-turn or “this answer only” preferences should stay session-local unless repeated or explicitly promoted
...
- whether a current-turn preference was intentionally kept `session-local only` / `not promoted`
...
## active package Release Adaptation Promotion Guard

This active package release addendum prevents prompt-optimization material from becoming unsafe default promotion.

- One good result is not enough to promote a reusable default.
- Repeated corrections are learning signals, not automatic memory or policy changes.
- Current explicit instruction and current evidence outrank stale memory.
- Adaptation promotion needs evaluation evidence, rollback condition, and scoped applicability.
- Failed adaptations should be rolled back, quarantined, or downgraded rather than silently retained.
