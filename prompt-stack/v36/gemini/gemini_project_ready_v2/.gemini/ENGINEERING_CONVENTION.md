...
This document defines repository-wide engineering conventions for code written or changed by the Gemini runtime agent.
...
If this document conflicts with `GEMINI.md`, `GEMINI.md` wins. If a narrower component document gives stricter rules for a specific subsystem, follow the stricter local rule unless it conflicts with `GEMINI.md`.
...
## 8. Gemini-Facing Code

- Preserve native Gemini concepts when code touches Gemini behavior: `systemInstruction`, `contents` with role-scoped `parts`, `functionDeclarations`, `functionCallingConfig`, structured output schemas, and `safetySettings`.
- Label OpenAI compatibility behavior as compatibility behavior and do not use it as native Gemini proof.
- Treat function calls as model requests; the runtime owns tool approval, execution, validation, redaction, and reinjection.
...
- A passing unit test proves only the tested behavior in that environment; do not claim CI, provider, adapter, release, or production readiness from local tests.
