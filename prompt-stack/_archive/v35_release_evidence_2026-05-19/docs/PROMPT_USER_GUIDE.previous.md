...
- harness engineering doctrine / activation level / trace-first loop -> `04_harness/PROMPT_harness_engineering.md`
- harness component contract / Guide-Sensor-Runner-Simulator-Sandbox-Telemetry-Gate split -> `04_harness/PROMPT_harness_contracts.md`
- harness release language / downgrade / gate precondition -> `04_harness/PROMPT_harness_release_gate.md`
- harness coverage / runner contract / sandbox policy / telemetry schema / trace-to-eval -> `04_harness/*` + `harness/*.json` + `docs/*`
- managed/config/code-defined/runtime-substrate distinction -> `04_harness/*` + `harness/runtime_substrate_contract.json` + `docs/agent-runtime-os.md`
- policy-before-action / observability-during-action / evaluation-after-action triangle -> `04_harness/*` + `harness/policy_eval_observability_triangle.json` + `docs/agent-runtime-os.md`
- tool surface quality / typed status / scope / idempotency review -> `04_harness/*` + `harness/tool_surface_quality_harness.json` + `docs/agent-runtime-os.md`
- containment architecture / sandbox escape readiness / misconfiguration review -> `04_harness/*` + `harness/sandbox_policy.json` + `harness/sandbox_escape_harness.json` + `docs/agent-runtime-os.md`
- initializer artifact / long-running handoff integrity -> `04_harness/*` + `harness/long_running_initializer_harness.json`
- claim strength wording and downgrade gate -> `04_harness/*` + `harness/claim_strength_gate.json` + `docs/agent-runtime-os.md`
- repository legibility / map-like AGENTS / repo-local docs contract -> `04_harness/*` + `AGENTS.md` + `harness/repository_legibility_harness.json` + `docs/agent-runtime-os.md`
- documentation freshness / doc-linter / doc-gardening policy -> `04_harness/*` + `harness/documentation_freshness_policy.json` + `docs/agent-runtime-os.md`
- agent-readable observability / reproduction / DOM-screenshot-log-metric-trace access -> `04_harness/*` + `harness/observability_harness.json` + `docs/agent-runtime-os.md`
- architecture invariant / deterministic boundary enforcement -> `04_harness/*` + `harness/architecture_invariant_harness.json` + `docs/agent-runtime-os.md`
- harness failure taxonomy / owner-fix routing -> `04_harness/*` + `harness/failure_classification.json` + `docs/agent-runtime-os.md`
- entropy control / garbage collection loop -> `04_harness/*` + `harness/garbage_collection_policy.json` + `docs/agent-runtime-os.md`
- PR risk class / throughput-aware review / merge philosophy -> `04_harness/*` + `harness/review_merge_policy.json` + `docs/agent-runtime-os.md`
- end-to-end agent bug or feature loop -> `04_harness/*` + `harness/e2e_task_harness.json` + `docs/agent-runtime-os.md`
- human taste encoding / repeated review comment promotion -> `04_harness/*` + `harness/human_taste_encoding.json` + `docs/agent-runtime-os.md`
- agent-first technology choice / legibility-aware tech review -> `04_harness/*` + `harness/agent_first_technology_review.json`
- harness readiness checklist / pre-release harness gate -> `04_harness/*` + `harness/harness_readiness_checklist.json` + `docs/agent-runtime-os.md`
- agent runtime operating system charter / autonomy level / component map -> `docs/agent-runtime-os.md` + `harness/runtime_os_charter.json`
- runtime component owner / activation / failure mode map -> `docs/agent-runtime-os.md` + `harness/runtime_component_map.json`
- context pack / active slice / discarded context rule -> `docs/agent-runtime-os.md` + `harness/context_pack_schema.json`
- tool capability registry / MCP capability registry -> `docs/agent-runtime-os.md` + `harness/tool_capability_registry.json` + `harness/mcp_capability_registry.json`
- policy control plane / default deny / approval-required action -> `docs/agent-runtime-os.md` + `harness/policy_rule_set.json`
- sandbox containment / code-execution separation / misconfiguration test -> `docs/agent-runtime-os.md` + `harness/sandbox_policy.json` + `harness/sandbox_escape_harness.json`
- runtime OS verification scenario set -> `docs/agent-runtime-os.md` + `harness/runtime_os_scenarios.json`
- prompt runtime behavior verification protocol -> `docs/prompt-runtime-verification.md` + `harness/prompt_runtime_verification_protocol.json`
- prompt behavior release threshold / hold / reject rule -> `docs/prompt-runtime-verification.md` + `harness/prompt_behavior_release_gate.json`
- mock tool suite and parameter / partial-state assertions -> `docs/agent-runtime-os.md` + `harness/mock_tool_contracts.json`
- behavior verification scenario families and canonical roster -> `docs/prompt-runtime-verification.md` + `harness/prompt_runtime_verification_protocol.json` + `harness/runtime_os_scenarios.json`
- retained validation reports only -> `validation/`
...
- `light review memo`: lookup, packet inventory, scoping, pre-flight review에 쓴다. 이 자체만으로 runner, controller, release gate 실행을 증명하지는 않는다.
- `stronger packet`: scored review, execution state, lifecycle state가 중요하지만 linked run 또는 controller evidence가 아직 최종 판단 경계는 아닐 때 쓴다.
- `operational artifact`: linked execution evidence, controller transition, promotion decision, drift investigation이 실제 control problem일 때 쓴다. replay, rollback, route-switch, release, controller-grade review의 active surface다.
...
artifact escalation, packet floor, downgrade, join rejection, split verdict retention, lineage preservation, failure-flow triage가 실제 쟁점일 때는 lower layer를 보기 전에 아래 블록부터 읽는다.

`1.6`과 `1.8`은 같이 읽는다. `1.6`은 active artifact ladder를 고르고, `1.8`은 required / recommended / optional packet coverage, downgrade language, recommended companion이 없을 때의 약한 언어를 결정한다.

### 1.6 제어 표면별 escalation matrix
...
### 1.8 직접 packet floor matrix / claim-language gate
...
### 1.10 Agent-first harness 빠른 gate

`v35`가 OpenAI식 harness engineering을 충분히 반영했는지가 실제 질문이라면 다음 순서로 본다.

1. is the repository map legible to the agent
2. are docs linked and fresh enough to trust
3. can the agent observe failure and success through logs, metrics, traces, UI, or runtime artifacts
4. are important architecture constraints deterministic rather than prose-only
5. is failure classified as prompt vs harness failure before rewrite
6. is entropy control or garbage collection defined
7. is PR risk class and merge philosophy explicit
8. can the agent run an end-to-end reproduce -> fix -> verify loop
9. are repeated human review comments being promoted into reusable rules
10. does the release claim stop at harness-readiness if execution evidence is still missing
11. is the runtime substrate only config-ready or actually code-defined / managed as claimed
12. are policy, observability, and evaluation connected as one loop
13. is sandbox containment stronger than mere sandbox existence
14. does wording stop at plausible or local when integration or runner evidence is absent
...
### 2.2 Overlay는 필요한 control surface에만 추가
...
### 2.4 Host-runtime 계층은 환경이 요구할 때만 추가
...
- coding-agent path라면 human brief, external knowledge inputs, quality-gate owner가 briefing surface에 실제로 보이는가
...
### 3.1 최소 runtime bundle

조립된 runtime이 단순 설명이 아니라 실제 실행까지 담당한다면, 최소한 다음 slot은 읽히게 유지한다.
...
- base prompt는 하나만 쓴다
- overlay는 필요한 surface에만 붙인다
- example layer는 구조 이득으로만 쓴다
- host-runtime layer는 execution environment fit일 때만 쓴다
...
- `PROMPT_light`
- evidence-sensitive하면 `PROMPT_retrieval_grounding_overlay`
- tool use가 필요하면 `PROMPT_tool_protocol_overlay`
...
- disclosure boundary가 중요하면 `PROMPT_guardrails_safety_overlay`
- `grounded-research`

추가로 다음 상황이면 overlay를 더 붙인다.

- tool-mediated retrieval, MCP capability reuse, internal source access -> `PROMPT_tool_protocol_overlay`
- reusable checkpoint continuity가 필요한 multi-round research -> `PROMPT_memory_adaptation_overlay`
- repeated retrieval quality comparison 또는 intermediate research gate -> `PROMPT_evaluation_monitoring_overlay`
...
### 5.5 멀티 에이전트 / orchestration

- `PROMPT_full`
- `PROMPT_multi_agent_overlay`
- `PROMPT_tool_protocol_overlay`
- `PROMPT_retrieval_grounding_overlay`
- coordination risk가 의미 있게 크면 `PROMPT_guardrails_safety_overlay`
- `AGENTS.md`
...
### 5.6 평가 / release review

- `PROMPT_full`
- `PROMPT_evaluation_monitoring_overlay`
- `PROMPT_search_reasoning_overlay`
- tool workflow 자체를 평가하면 `PROMPT_tool_protocol_overlay`
- `eval-ops`

### 5.7 memory / adaptation 중심 runtime
...
packet-heavy path 조립 알림:

- `Packet compliance report`를 열기 전에, 가이드의 direct packet floor matrix에서 `required / recommended / optional packet` ownership을 먼저 결정한다
- required packet은 있지만 recommended companion이 없으면, 더 약한 companion-missing language를 명시한다
- `Packet compliance report`는 observed-vs-required review, omission findings, packet coverage audit에만 쓴다
