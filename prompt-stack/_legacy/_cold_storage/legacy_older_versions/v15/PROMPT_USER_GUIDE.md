...
이 정리는 v15 프롬프트 스택을 실제로 오래 보관하고, 목적별로 바로 꺼내 쓰기 쉽게 정리하기 위한 폴더 구조 가이드입니다.

핵심 목적은 다음과 같습니다.

1) 원본 14개 문서를 역할별로 분리 보관
...
현재 실제 보유한 파일 14개만 기준으로 분류하고 복사 배치하는 구조입니다.

================================================================
2. 현재 실제 보유 파일 목록
================================================================

현재 실제 보유 파일은 아래 14개입니다.

[governance]
- PROMPT_guideline.txt

[base execution prompts]
- PROMPT_full.txt
- PROMPT_light.txt
- PROMPT_lightest.txt
- PROMPT_standalone.txt

[examples]
- PROMPT_example_injection.txt
- PROMPT_example_catalog.txt

[overlays]
- PROMPT_memory_adaptation_overlay.txt
- PROMPT_tool_protocol_overlay.txt
- PROMPT_multi_agent_overlay.txt
- PROMPT_search_reasoning_overlay.txt
- PROMPT_retrieval_grounding_overlay.txt
- PROMPT_evaluation_monitoring_overlay.txt
- PROMPT_guardrails_safety_overlay.txt

즉, 아래 모든 폴더 구조는 이 14개 파일만 이동/복사해서 구성하는 방식입니다.

================================================================
3. 먼저 이해해야 할 기본 원칙
================================================================

이 v15 스택은 “문서 14개를 항상 한 번에 전부 넣는 구조”가 아닙니다.
...
- 안전 제약, tool restriction, containment, review coupling이 중요하면 guardrails_safety_overlay
...
3) PROMPT_lightest.txt
- 매우 단순한 작업
- 토큰/지연 제약이 큰 환경
- fallback 용도
- 설명보다 신속성과 최소 orchestration이 중요할 때

4) PROMPT_standalone.txt
- 코딩 에이전트용 압축 실행 헌법
...
1) PROMPT_example_injection.txt
- example을 언제 쓸지
- 어떤 example이 맞는지
- skeleton으로 쓸지
...
5) overlay는 필요한 것만
- retrieval / search / tool / memory / multi-agent / evaluation / guardrails 중 선택
