...
### 1.8 Claim-language gate

| Claim surface | Minimum packet floor before the claim | Downgrade language if the floor is missing |
| --- | --- | --- |
| `benchmark-grade` | `Benchmark cohort manifest` with stable lineage; if executed comparability is referenced, keep linkable `Benchmark execution report` evidence | cohort-defined, registry-only, execution-summary |
| `replay-grade` | `Replay runner verdict sheet` | replay-planned, replay-attempted, partial replay |
| `controller-grade` | `Adaptation controller audit packet` | lifecycle summary, adaptation tendency, promotion review only |
| `coding-proof-grade` | `Coding benchmark execution ledger` | local plausibility, proof bundle only, partial validation |
| `release-grade` | `Release promotion decision record` with compatible underlying lineage | release recommendation, evidence review, hold/propose-only |
| `drift-grade` | `Telemetry drift investigation memo` | telemetry trend, anomaly suspicion, follow-up needed |
| `route-quality-grade` | `Route-switch benchmark verdict` | advisory route note, reprioritization audit, exploration note |
| `retrieval-substrate-grade` | `Context substrate scorecard` | context weakness note, taxonomy-only diagnosis, stale-context caution |

Optional direct gate:

- if critique utility itself must be proven rather than praised, use `Critique delta ledger`; otherwise stay at `Critique utility scorecard` or lighter critique review language
