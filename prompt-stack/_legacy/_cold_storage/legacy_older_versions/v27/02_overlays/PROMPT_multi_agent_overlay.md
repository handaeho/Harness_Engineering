...
- preserve stable `trace_id`, `run_id`, handoff owner, and join state when the same topology is compared across cohorts or releases
...
- if multiple coordination artifacts are merged, preserve an explicit join rule for artifact precedence, unresolved join failure, and downgrade behavior
