...
- if the task has crossed from packet presence review into operational proof review, prefer the stronger v27 artifact and mark weaker packets as superseded rather than silently doubling them
- keep shared identifiers such as `scenario_id`, `run_id`, `cohort_id`, and `trace_id` stable whenever benchmark, replay, adaptation, release, or telemetry packets must be linked across rounds
