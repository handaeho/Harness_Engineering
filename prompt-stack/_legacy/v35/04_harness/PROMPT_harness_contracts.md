...
이 문서는 `v35` harness component contract를 정의한다.

## 1. Component Model

각 harness는 다음 요소를 가능한 한 분리해서 기술한다.
...
- guide만 있고 runner contract 없음
- runner 존재만으로 release-grade claim
- trace 없음에도 trajectory claim
- sandbox 없음에도 autonomy claim
- telemetry 없음에도 production-monitored claim
- repo map 없음에도 agent-first repo claim
- docs 존재만으로 docs fresh claim
- PR opened만으로 task complete claim
- repeated review taste를 사람이 계속 수동으로 반복함에도 harness maturity claim
- config harness만으로 code-defined or managed-runtime strength claim
- trace captured만으로 evaluation passed claim
- sandbox 존재만으로 containment verified claim

<!-- V35_RELEASE_STABLE_PATCH_START -->
## v35 Release Native Replay Contract

This v35 release addendum defines the Phase 4R replay contract.
...
- A replay runner must read v35 assets, not previous-baseline paths, when claiming v35 evidence.
